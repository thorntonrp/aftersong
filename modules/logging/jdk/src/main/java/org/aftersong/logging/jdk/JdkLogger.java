...
class JdkLogger extends Logger {
			...
			if (caller != null) {
				...
				if (in != null) {
					try {
						...
						java.util.logging.Logger.getLogger(JdkLogger.class.getName()).log(Level.SEVERE,
	 ...
	 */
	public static JdkLogger getLogger() {
		Class<?> caller = CallStackProvider.getInstance().getCaller();
		if (caller != null) {
			return new JdkLogger(java.util.logging.Logger.getLogger(caller.getName()));
		} else {
			java.util.logging.Logger.getLogger(JdkLogger.class.getName()).log(Level.WARNING,
	 ...
	 */
	public static JdkLogger getLogger(Class<?> forClass) {
		return new JdkLogger(java.util.logging.Logger.getLogger(forClass.getName()));
	}
	 ...
	 */
	public static JdkLogger getLogger(Package forPackage) {
		return new JdkLogger(java.util.logging.Logger.getLogger(forPackage.getName()));
	}
	 ...
	 */
	public static JdkLogger getLogger(String loggerName) {
	...
	protected JdkLogger(java.util.logging.Logger logger) {
		delegate = logger;
	}
	...
	private StackTraceElement getCallingStackTraceElement() {
		try {
			...
			String logClassName = JdkLogger.class.getName();
			int index;
	...
	private static class CallStackProvider extends SecurityManager {
		...
		Class<?> getCaller() {
			for (Class<?> caller : getCallStack()) {
				...
				if (!callerName.equals(JdkLogger.class.getName())) {
					return caller;
				}
			}
			return null;
